    ...
    var instance = null,
      clazz = declare(null, {
        ...
        _addDataLoadingOnMapUpdate: function (map) {
          var loadHtml = '<img class="spinner" src="images/gifQuindio.gif" alt="SIG QUINDIO"/>';
          var loadContainer = html.toDom(loadHtml);
          html.place(loadContainer, map.root);
          if (map.updating) {
            html.addClass(loadContainer, 'spinner');
          }
          on(map, 'update-start', lang.hitch(this, function () {
            html.addClass(loadContainer, 'spinner');
          }));
          on(map, 'update-end', lang.hitch(this, function () {
            html.removeClass(loadContainer, 'spinner');
          }));
          on(map, 'unload', lang.hitch(this, function () {
            html.destroy(loadContainer);
            loadContainer = null;
          }));
          on(map, 'extent-change', (this, function (event) {

            var maxExtent = new esri.geometry.Extent({
              "xmax": 1423239.2771299225,
              "xmin": 904238.2611278904,
              "ymax": 1126180.6275840316,
              "ymin": 832340.4785704,
              "spatialReference": { "wkid": 3115 }
            })
