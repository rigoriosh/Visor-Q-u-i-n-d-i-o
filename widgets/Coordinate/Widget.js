    ...
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      */
      ...
      _displayCoordinatesByOrder: function (x, y) {
        ...
        //let gc = webMercatorUtils.webMercatorToGeographic(event.mapPoint);
        // console.log(DDtoDMS(gc.x, 'LONG'));
        // console.log(DDtoDMS(gc.y, 'LAT'));

        let xFix = parseFloat(x.replace(/\./g,'').replace(',','.'));
        ...
        let punto = new Proj4js.toPoint([xFix,yFix]);  
        let gc =  Proj4js.transform(source, dest, punto); 

        this.coordinateInfo.innerHTML = 'Lat = ' + this.degToDMS(gc.y, 'LAT') + ' Long = ' + this.degToDMS(gc.x, 'LON')

      },

      DDtoDMS: function (deg, degDir) {
        ...
        let dir = (degDir === 'LAT') ? (deg < 0 ? "S" : "N") : (deg < 0 ? "O" : "E");
      ...
      degToDMS: function (decDeg, decDir) {
        ...
        /** @type {string} */
        var dir = (decDir === 'LAT') ? (decDeg < 0 ? "S" : "N") : (decDeg < 0 ? "O" : "E");
